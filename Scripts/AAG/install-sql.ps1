...
$Aruments = "/q /ACTION=Install /FEATURES=SQL,SSMS $Diskparameter /INSTANCENAME=MSSQL$Domain /SQLSVCACCOUNT=`"$Domain\svc_sqladm`" /SQLSVCPASSWORD=`"Password123!`" /SQLSYSADMINACCOUNTS=`"$Domain\svc_sqladm`" `"$Domain\Administrator`" `"$Domain\sql_admins`" /AGTSVCACCOUNT=`"NT AUTHORITY\Network Service`" /IACCEPTSQLSERVERLICENSETERMS"
Write-Verbose $Aruments
Start-Process $Setuppath -ArgumentList  $Aruments -Wait
Set-Content -Value $LASTEXITCODE -Path $Builddir\sqlexit.txt
# /SQLUSERDBDIR=`"m:\`" /SQLUSERDBLOGDIR=`"n:\`" /SQLTEMPDBDIR=`"o:\`" /SQLTEMPDBLOGDIR="p:\""
