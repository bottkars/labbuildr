    ...
    If ($newdeploy)
	    ...
	    if ($ScaleIO.IsPresent)
            ...
            if ($singlemdm.IsPresent)
                    ...
                    get-vmx $FirstVMX | invoke-vmxpowershell -Guestuser $Adminuser -Guestpassword $Adminpassword -ScriptPath $Targetscriptdir -Script configure-mdm.ps1 -Parameter "-singlemdm -CSVnum 2" -interactive 
                    }
            else
                    {
                    Write-Warning "Configuring Clustered MDM"
                    get-vmx $FirstVMX | invoke-vmxpowershell -Guestuser $Adminuser -Guestpassword $Adminpassword -ScriptPath $Targetscriptdir -Script configure-mdm.ps1 -Parameter "-CSVnum 2" -interactive 
                    }
            }
		if ($SCVMM.IsPresent)
		    {
