...
curl --silent --show-error --insecure --user :`$TOKEN -X POST -H 'Content-Type: multipart/form-data' -F 'file=@/tmp/mdm_b.cer' 'https://localhost/api/trustHostCertificate/Mdm' "
	...
	if ($Openstack_Controller.IsPresent)
		...
		$Scriptblock = "cd /mnt/hgfs/Scripts/openstack/Controller; sh ./install_base.sh -spd $ProtectionDomainName -ssp $StoragePoolName -sgw $tb_ip -d $BuildDomain -c $($Openstack_Baseconfig.ispresent.ToString().tolower())"
