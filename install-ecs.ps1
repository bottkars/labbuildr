...
[Parameter(ParameterSetName = "defaults", Mandatory = $false)]
[Parameter(ParameterSetName = "install",Mandatory=$false)]
[ValidateSet("release-2.1","Develop",'master','latest','2.2.0.1','2.2.0.2')]$Branch = 'master',
...
foreach ($Node in $machinesBuilt)
        {
        Write-Host -ForegroundColor Magenta " ==>Configuring GuestOS Network"
...
if (!(Test-Path "$Sourcedir\docker\$($Docker_image)_$Docker_imagetag.tgz") -and !($offline.IsPresent))
    ...
    Write-Verbose $Scriptblock
    $Bashresult = $NodeClone | Invoke-VMXBash -Scriptblock $Scriptblock -Guestuser $Rootuser -Guestpassword $Guestpassword -logfile $Logfile
    Write-Host -ForegroundColor Magenta " ==>Creating Offline Image for ECS $($Docker_imagename):$Docker_imagetag"
    ...
    $Scriptblock = "gunzip -c /mnt/hgfs/Sources/docker/$($Docker_image)_$Docker_imagetag.tgz | docker load"
