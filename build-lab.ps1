    ...
    If ($newdeploy)
	    ...
	    if ($ScaleIO.IsPresent)
            ...
            if ($singlemdm.IsPresent)
                    ...
                    get-vmx $FirstVMX | invoke-vmxpowershell -Guestuser $Adminuser -Guestpassword $Adminpassword -ScriptPath $IN_Guest_UNC_NodeScriptDir -Script configure-mdm.ps1 -Parameter "-subnet $IPv4Subnet -singlemdm -CSVnum 3 -ScaleIO_Major $ScaleIO_Major"-interactive 
                    }
            else
                    {
                    Write-Host -ForegroundColor Gray " ==> Configuring Clustered MDM"
                    get-vmx $FirstVMX | invoke-vmxpowershell -Guestuser $Adminuser -Guestpassword $Adminpassword -ScriptPath $IN_Guest_UNC_NodeScriptDir -Script configure-mdm.ps1 -Parameter "-subnet $IPv4Subnet -CSVnum 3 -ScaleIO_Major $ScaleIO_Major" -interactive 
                    }
            }
		if ($SCVMM.IsPresent)
		    {
