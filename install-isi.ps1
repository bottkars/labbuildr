...
If (!$MasterVMX)
    ...
    if (!(Test-Path (Join-Path $Sourcedir $sourcemaster )))
        ...
        $Bootdisk= Get-ChildItem -path $ISISource -recurse -Depth 0 -Filter "boot0.vmdk"
        Write-Verbose "Found Bootbank $($Bootdisk.fullname)"
        $nvramfile = Get-ChildItem -path $ISISource -recurse -Depth 0 -Filter "nvram"
        $nvramdisk = Get-ChildItem -path $ISISource -recurse -Depth 0 -Filter "isi-nvram.vmdk"
        $vmxfile = Get-ChildItem -path $ISISource -recurse -Depth 0 -Filter "b.7.*vga.vmx"
