...
[Parameter(ParameterSetName = "defaults", Mandatory = $false)]
[Parameter(ParameterSetName = "install", Mandatory=$False)][ValidateSet('36GB','72GB','146GB')][string]$Disksize = "36GB",
...
If (!$MasterPath)
    ...
    $MasterVMXs = get-vmx -vmxname "ISISDMaster*"
    if ($Mastervmxs)
            {
            ...
            {
            $sourcemaster = "EMC_IsilonSD_Simulator*"
            }
    }
else
            {
...
If (!$MasterVMX)
    ...
    if (!(Test-Path (Join-Path $Sourcedir $sourcemaster )))
            { 
            if (!(Test-Path (Join-path $Sourcedir "EMC*isilonSD*.zip")))
                {
        ...
        $Isiverlatest = $Isiver -replace "EMC_IsilonSD_Simulator_",""
        ...
        $isijournalfile = Get-ChildItem -path $ISISource -Filter "isi-journal.vmdk"
        # $nvramdisk = Get-ChildItem -path $ISISource -Filter "isi-nvram.vmdk"
        $vmxfile = Get-ChildItem -path $ISISource -Filter "EMC_IsilonSD*.vmx"
        $Masterpath = ".\ISISDMaster$Isiverlatest"
        ...
        pause
        Copy-Item ($Bootdisk.FullName,$isijournalfile.FullName,$vmxfile.FullName ) -Destination $MasterPath
