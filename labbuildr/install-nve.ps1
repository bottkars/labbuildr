...
switch ($PsCmdlet.ParameterSetName)
{
    "import"
        ...
        if (!($Importfile = Get-ChildItem -Path $nve_dir -Filter "$Product_tag.ovf" -ErrorAction SilentlyContinue))
            ...
            Receive-LABNetworker -nve -nve_ver $nve_ver -Destination "$Sourcedir" -Confirm:$false
            $OVA_File = Get-ChildItem -Path "$Sourcedir" -Recurse "*$Product_tag.ova "-Exclude ".*" | Sort-Object -Descending | Select-Object -First 1
            if (!$OVA_File)
				{
	...
	foreach ($node in 1..$Nodes)
		...
		if (!($MasterVMX = Get-VMX -Path $masterpath -VMXName  "NVE-$Product_tag"))
			{
